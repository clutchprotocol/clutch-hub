...
- `main.jsx` — entry: StrictMode, imports `./pwaUpdate` and the leaflet/fontsource/material-symbols CSS.
- `pwaUpdate.js` — service worker registration, outside React because it must happen once per document rather than once per mount. Exports `subscribeToUpdate` and `reloadForUpdate` for `components/UpdatePrompt.jsx`.
...
- Styling: hand-written CSS. `App.css` is now imports only; the rules live in `src/styles/*.css` (shell, entry, wallet, primitives, map, sections, explorer, forms, trip, responsive, overlays) plus the `index.css` reset, which holds the CSS variables. **One palette, light.** Dark mode was removed on 2026-09-13 — there is no `data-theme` attribute, no toggle and no `useTheme` hook, and the map has one tile URL rather than a light and a dark one. **The import order in `App.css` is load-bearing** — cascade is source order, and `responsive.css` overrides rules defined above it, so a new sheet goes where its rules belong in that sequence, not at the end. Lots of inline `style={{}}` for one-offs; reuse classes like `card`, `btn-primary`, `status-banner`, `form-row`.
- PWA (`vite-plugin-pwa`) is enabled in dev too — `dev-dist/` is generated service-worker output, don't edit it.
- **`registerType` is `'prompt'`, not `'autoUpdate'`, and that is deliberate.** autoUpdate reloads the page the instant a new build activates, which here can mean losing a half-entered fare or a pickup pin. `UpdatePrompt` shows a persistent banner with a Reload button instead. It does not fade: the old build keeps working so there is no urgency, but there is also no second chance to mention it.
- **The cache headers in `nginx.conf` are load-bearing, and the edge fights them.** Measured 2026-09-13: the origin sent `no-cache` for `/sw.js` and the browser received `max-age=14400`, because Cloudflare caches by extension, treated it as an ordinary `.js` asset and stamped its own 4-hour Browser Cache TTL over ours. A service worker can only find a new build by fetching `sw.js`, so that one header made the app unable to update itself at all — clearing site data was the only way out. `Cloudflare-CDN-Cache-Control: no-store` fixes it by keeping the edge from caching those documents in the first place; the browser TTL rewrite only applies to responses Cloudflare caches. If update behaviour ever looks wrong again, `curl -I` the live `/sw.js` and read `cf-cache-status` before touching any code.
